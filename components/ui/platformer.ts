...
// `o` bean, `i` candle (whip it for a bean), `b` bat, `s` skeleton, `P` start, `D` the door out.
...
function sub(g: Game, input: Input, dt: number) {
  ...
  else if (tilesIn(hero(g), (cx, cy) => tile(cx, cy) === "D")) { g.state = "won"; g.t = 0; }
}
